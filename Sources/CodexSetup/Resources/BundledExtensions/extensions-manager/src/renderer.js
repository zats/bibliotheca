function activate(context) {
  ...
  function createSwitch(extension) {
    ...
    toggle.className = `inline-flex items-center text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-token-focus-border focus-visible:rounded-full ${extension.compatible ? "cursor-interaction" : "cursor-not-allowed opacity-60"}`;
    toggle.disabled = !extension.compatible;
  ...
  function extensionDetail(extension) {
    if (!extension.compatible && extension.codexAppVersion) {
      return `Requires Codex ${extension.codexAppVersion}`;
    }
  ...
  async function renderExtensionsPage() {
    ...
    if (root.querySelector("[data-codex-extensions-settings='true']")) return;
    root.textContent = "";
