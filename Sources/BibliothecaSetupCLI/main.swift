...
struct BibliothecaSetupCLI {
    ...
    private static func run(arguments: [String]) async throws {
        ...
        let runtime = BibliothecaSetupRuntime()
        ...
        case "auto-repatch-updated":
            Self.printProgress("checking updated Codex")
            let outcome = try await runtime.repatchUpdatedCodexIfNeeded(appURL: appURL)
            Swift.print("auto-repatch: \(outcome)")
        case "prune-backups":
        ...
        case "uninstall":
            Self.printProgress("uninstalling Bibliotheca")
            try await runtime.uninstallBibliotheca(appURL: appURL)
    ...
    private static func printSnapshot(_ snapshot: BibliothecaSetupSnapshot) {
    ...
    private static func describe(_ status: BibliothecaStoreStatus) -> String {
        if !status.exists {
            return "missing"
        }
    ...
    private static func printHelp() {
        Swift.print("""
        usage: bibliotheca-setup <command> [--app /Applications/Codex.app]
