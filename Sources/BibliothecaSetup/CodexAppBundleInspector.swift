...
struct CodexAppBundleInspector: Sendable {
    ...
    init(fileSystem: BibliothecaSetupFileSystem, asarIdentityCache: CodexASARIdentityCache = CodexASARIdentityCache()) {
    ...
    func identity(at appURL: URL) throws -> CodexAppIdentity {
        ...
        guard self.fileSystem.fileExists(at: infoURL) else {
            throw BibliothecaSetupError.invalidCodexBundle(appURL)
        }
...
extension BibliothecaSetupConfiguration {
    static func liveDefault() -> Self {
        ...
        return Self(
            ...
            provisioningReceiptURL: homeURL.appending(path: ".codex/extensions/.bibliotheca-receipt.json"),
