...
public struct BibliothecaSetupRuntime: Sendable {
    private let service: BibliothecaSetupService

    public init() {
        self.service = BibliothecaSetupService()
    }

    public func inspect(appURL: URL? = nil, checkForUpdates: Bool = false) async -> BibliothecaSetupSnapshot {
    ...
    public func uninstallBibliotheca(appURL: URL? = nil) async throws {
        let snapshot = try await self.requiredSnapshot(appURL: appURL)
        try self.service.uninstallBibliotheca(appURL: snapshot.appURL)
    }

    public func performRecommendedAction(appURL: URL? = nil) async throws -> BibliothecaSetupRecommendedAction {
        ...
        case .uninstallBibliotheca:
            try self.service.uninstallBibliotheca(appURL: values.appURL)
        }
        return values.snapshot.recommendedAction
    }

    private func requiredSnapshot(appURL: URL?, checkForUpdates: Bool = false) async throws -> (snapshot: BibliothecaSetupSnapshot, appURL: URL, appIdentity: CodexAppIdentity) {
        ...
        guard let appURL = snapshot.appURL, let appIdentity = snapshot.appIdentity else {
            throw BibliothecaSetupError.codexAppMissing
        }
