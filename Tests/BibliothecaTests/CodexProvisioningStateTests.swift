...
struct CodexProvisioningStateTests {
    ...
    func cleanRestoreCleanupKeepsExtensionDataAndRemovesReceiptAndBackups() throws {
        ...
        let receiptURL = extensionsURL.appending(path: ".bibliotheca-receipt.json")
        let backupURL = extensionsURL.appending(path: ".bibliotheca-backups/current", directoryHint: .isDirectory)
        ...
        let service = BibliothecaSetupService(
            configuration: BibliothecaSetupConfiguration(
                candidateAppURLs: [],
    ...
    func failedCleanRestoreKeepsProvisioningReceiptAndExtensionData() async throws {
        ...
        } catch BibliothecaSetupError.updateFeedMalformed {
        } catch {
