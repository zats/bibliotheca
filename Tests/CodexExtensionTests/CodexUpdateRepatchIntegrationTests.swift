...
struct CodexUpdateRepatchIntegrationTests {
    ...
    func repatchesCleanCodexCopyAfterProvisionedUpdate() async throws {
        ...
        let liveConfiguration = CodexSetupConfiguration.liveDefault()
        let configuration = CodexSetupConfiguration(
        ...
        let service = CodexSetupService(configuration: configuration, fileSystem: fileSystem)
    ...
    private static func cleanBackupDirectory() throws -> URL {
        let backupRoot = URL(filePath: NSHomeDirectory(), directoryHint: .isDirectory)
            .appending(path: ".codex/extensions/.codex-extension-backups", directoryHint: .isDirectory)
        let fileSystem = LocalCodexSetupFileSystem()
