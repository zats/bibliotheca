...
@Suite
struct CodexExtensionStoreInstallerTests {
    @Test
    func firstInstallDisablesUserExtensionsAndEnablesManager() throws {
        ...
        let installer = CodexExtensionStoreInstaller(fileSystem: fileSystem)
    ...
    private static func makeRoot() throws -> URL {
        let rootURL = URL(filePath: NSTemporaryDirectory(), directoryHint: .isDirectory)
            .appending(path: "codex-extension-store-test-\(UUID().uuidString)", directoryHint: .isDirectory)
